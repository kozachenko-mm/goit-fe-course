...
class Hamburger {
  ...
  calculatePrice() {
    ...
    let priceTopping = this._toppings.reduce(
      (acc, el) => Hamburger.TOPPINGS[el].price + acc,
      0
    );
    let priceStuf = Hamburger.STUFFINGS[this._stuffing].price;
    return priceSize + priceStuf + priceTopping;
  }

  //  Узнать калорийность
   calculateCalories() {
    let caloriesSize = Hamburger.SIZES[this._size].calories;
    let caloriesTopping = this._toppings.reduce(
      (acc, el) => Hamburger.TOPPINGS[el].calories + acc,
      0
    );
    let caloriesStuf = Hamburger.STUFFINGS[this._stuffing].calories;
    return caloriesSize + caloriesStuf + caloriesTopping;
  }
}
